...
public class OfflineAssetManagerWindow : BaseWindow
{
    ...
    {
        OfflineAssetManager.UpdateIndexFromFilesInFolder("index-template.json".ToNPath(), "index.json".ToNPath(), "/code/projects/spoiledcat/webserver/files/".ToNPath());
    }
...
#else
                    var msg = $"{progress.Message} {(int)(progress.Percentage * 100)}%";
                    if (progress.InnerProgress != null)
                    {
                        msg += $" - {progress.InnerProgress.Message} {(int)(progress.InnerProgress.Percentage * 100)}%";
                    }
                    logger.Info(msg);
#endif
                });
...
#else
                var msg = $"{(int)(progress.Percentage * 100)}%";
                if (progress.InnerProgress != null) {
                    msg += $" - {progress.InnerProgress.Message} {(int)(progress.InnerProgress.Percentage * 100)}%";
                }
                if (progress.InnerProgress?.InnerProgress != null) {
                    msg += $" - {progress.InnerProgress.InnerProgress.Message} {(int)(progress.InnerProgress.InnerProgress.Percentage * 100)}%";
                }
                logger.Info(msg);
#endif
            });
