...
namespace SpoiledCat.Json
{
 ...
 interface IJsonSerializerStrategy
	...
	class PocoJsonSerializerStrategy : IJsonSerializerStrategy
	{
		...
		private static Dictionary<Type, Func<object, (object, bool)>> CustomSerializationHandlers;
		private static Dictionary<Type, Func<object, Type, (object, bool)>> CustomDeserializationHandlers;
		...
		/// <summary>
		/// Register a handler that knows how to serialize types that SimpleJson doesn't know about
		/// </summary>
		/// <param name="handler"></param>
		public static void RegisterCustomTypeHandler(Type type,
			Func<object, (object, bool)> serializationHandler,
			Func<object, Type, (object, bool)> deserializationHandler)
		{
			if (CustomSerializationHandlers == null)
				CustomSerializationHandlers = new Dictionary<Type, Func<object, (object, bool)>>();
			CustomSerializationHandlers.Add(type, serializationHandler);

			if (CustomDeserializationHandlers == null)
				CustomDeserializationHandlers = new Dictionary<Type, Func<object, Type, (object, bool)>>();
			CustomDeserializationHandlers.Add(type, deserializationHandler);
		}

		public static void RegisterCustomTypeHandler<T>(
			Func<object, (object, bool)> serializationHandler,
			Func<object, Type, (object, bool)> deserializationHandler)
		{
			RegisterCustomTypeHandler(typeof(T), serializationHandler, deserializationHandler);
		}

		public virtual bool TrySerializeNonPrimitiveObject(object input, out object output)
		{
			if (CustomSerializationHandlers != null && CustomSerializationHandlers.TryGetValue(input.GetType(), out Func<object, (object, bool)> handler)) {
				var (ret, success) = handler(input);
				if (success) {
					output = ret;
					return true;
				}
		...
		public object DeserializeObject(object value, Type type)
		{
			...
			if (CustomDeserializationHandlers != null && CustomDeserializationHandlers.TryGetValue(type, out Func<object, Type, (object, bool)> handler)) {
				var (ret, success) = handler(value, type);
				if (success)
					return ret;
			}
 ...
 class DataContractJsonSerializerStrategy : PocoJsonSerializerStrategy
	{
		...
		static class ReflectionUtils
		{
			...
			public static bool IsNullableOfType(Type thisType, Type isOfThisType)
			{
				return thisType.IsNullableType() && Nullable.GetUnderlyingType(thisType) == isOfThisType;
			}

			public static bool IsOfType(Type thisType, Type isOfThisType, bool strict = false)
			{
				return thisType == isOfThisType || (!strict && IsNullableOfType(thisType, isOfThisType));
			}

			public static Type GetTheType(Type type)
			{
