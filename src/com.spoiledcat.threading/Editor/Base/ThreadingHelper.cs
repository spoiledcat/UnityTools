...
{
	using static SpoiledCat.Threading.ThreadingHelper;

	public static class AwaitExtensions
	{
		public static IAwaitable AwaitInline(this Task task) => new AwaitableWrapper(new ExecuteContinuationSynchronouslyAwaiter(task));

        public static IAwaitable AwaitOn(this Task task, ITaskManager taskManager, TaskAffinity affinity) => new AwaitableWrapper(taskManager.GetScheduler(affinity));

        public static IAwaitable SwitchTo(this TaskScheduler scheduler) => new AwaitableWrapper(scheduler);
	}
}
