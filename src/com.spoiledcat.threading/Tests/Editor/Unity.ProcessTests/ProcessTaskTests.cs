...
namespace ProcessManagerTests
{
	...
	public partial class ProcessTaskTests : BaseTest
	{
		...
		public IEnumerator CanRunProcessOnMono()
		{
			using (var test = StartTest())
			{
				var monoPath = "MonoBleedingEdge/bin/mono" + test.Environment.ExecutableExtension;
				var hasMono = test.Environment.UnityApplicationContents.IsInitialized;

				if (!hasMono || !test.Environment.UnityApplicationContents.Combine(monoPath).FileExists())
				{
